...
func (a *Auth) form(w http.ResponseWriter, r *http.Request, title, action, message string, recovery bool, status int) {
	...
	templ.Handler(pages.AuthForm(title, action, webmiddleware.RedirectTarget(r.FormValue("next")), csrf, message, recovery)).ServeHTTP(w, r)
}
...
func (a *Auth) submit(kind string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		http.Redirect(w, r, webmiddleware.RedirectTarget(r.PostFormValue("next")), http.StatusSeeOther)
	}
}
